...
const source = ART+'shijun-detail/'
...
export default function ShijunDetail({ tab,onTab,onBack,onHome }: {
  tab:number; onTab:(tab:number)=>void; onBack:()=>void; onHome:()=>void
}) {
  const motionRef = useDetailMotion(tab)
  return <section ref={motionRef} className="lh-detail" aria-label="石峻角色详情">
    <img className="lh-background" src={ART+'details/background.jpg'} alt="" draggable={false}/>
    <img className="lh-art lh-portrait" style={place(1015,-144,995,1254)} src={source+'renwu04.png'} alt="山灵守者石峻，持山魂杖立于人物卷轴中" draggable={false}/>
    ...
    {tab === 1 && <div className="lh-attributes" role="region" aria-label="石峻技能属性">
      <img className="lh-art" style={place(93,143,457,421)} src={source+'6.png'} alt="等级12" draggable={false}/>
      <img className="lh-art" style={place(110,487,433,164)} src={source+'5.png'} alt="等级进度：LV.59/385，525/1000" draggable={false}/>
      <DetailStats character="shijun" style={place(530,278,669,495)} src={source+'4.png'} alt="石峻属性：生命值、灵力、攻击力、暴击率、速度"/>
      <img className="lh-art" style={place(0,600,621,246)} src={source+'1.png'} alt="灵息回涌，四星：群体治疗为核心，回复量可观，并有额外灵力回复机制，续航能力出众。" draggable={false}/>
      <img className="lh-art" style={place(559,706,622,246)} src={source+'3.png'} alt="磐石守护，五星：护盾量大，团队防御提升，稳定性极高，适合关键防御回合。" draggable={false}/>
      <img className="lh-art" style={place(0,787,621,246)} src={source+'2.png'} alt="山川不动，三星：群体治疗为核心，回复量可观，并有额外灵力回复机制，续航能力出众。" draggable={false}/>
    </div>}
    {tab !== 1 && <article className="lh-content lh-content--unavailable" aria-label={`石峻 · ${tabs[tab].name}`}>

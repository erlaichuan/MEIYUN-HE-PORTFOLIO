...
import './painterDetail.css'

const source = ART+'painter-detail/'
...
export default function PainterDetail({ hero,tab,onTab,onBack,onHome }: {
  hero:'moxiu' | 'mowan'; tab:number; onTab:(tab:number)=>void; onBack:()=>void; onHome:()=>void
}) {
  const character = characters.find(c => c.id === hero)!
  const motionRef = useDetailMotion(tab)
  return <section ref={motionRef} className="lh-detail painter-detail" aria-label={`${character.name}角色详情`}>
    <img className="lh-background" src={ART+'details/background.jpg'} alt="" draggable={false}/>
    <img className="lh-art lh-portrait" style={place(696,0,1227,1080)} src={source+(hero === 'mowan' ? '1-1.png' : '1.png')} alt={`画境师${character.name}，与青绿山水画卷相伴`} draggable={false}/>
    ...
    {tab === 1 && <div className="lh-attributes" role="region" aria-label={`${character.name}技能属性`}>
      <img className="lh-art" style={place(93,143,457,421)} src={source+'6.png'} alt="等级20" draggable={false}/>
      <img className="lh-art" style={place(110,487,433,164)} src={source+'7.png'} alt="等级进度：LV.132/500，525/1000" draggable={false}/>
      <DetailStats character="painter" style={place(530,278,669,495)} src={source+'2.png'} alt={`${character.name}属性：生命值、灵力、攻击力、暴击率、速度`}/>
      <img className="lh-art" style={place(0,600,621,246)} src={source+'4.png'} alt="妙笔生花，三星：挥笔生灵，以画中花木之力恢复队友生命。" draggable={false}/>
      <img className="lh-art" style={place(559,706,622,246)} src={source+'5.png'} alt="化墨为兵，四星：凝墨成刃，将画中之物化为兵器攻击敌人。" draggable={false}/>
      <img className="lh-art" style={place(0,787,621,246)} src={source+'3.png'} alt="丹青妙术，四星：展开丹青画卷，扰乱敌人并削弱其行动。" draggable={false}/>
    </div>}
    {tab !== 1 && <article className="lh-content lh-content--unavailable" aria-label={`${character.name} · ${tabs[tab].name}`}>

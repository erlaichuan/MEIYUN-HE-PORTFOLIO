...
import { framedPose, restPose } from './cameraPath'
import {
  activateHotspot,
  FOCUS_MS,
  movingHotspotAnchor,
 ...
 *      普通标签的指针交互交给 raycaster；工牌标签例外，直接接收点击，
 *      避免点穿到电脑。键盘 Tab / Enter / Space 仍触发同一个入口。
 */
...
export function CameraReturn() {
  ...
  useEffect(() => {
    ...
    flyTo(currentPose(), framedPose(restPose(), width / Math.max(1, height)), FOCUS_MS, () => {
      releaseCamera()
  ...
  useEffect(() => {
    ...
    const rest = framedPose(restPose(), width / Math.max(1, height))
